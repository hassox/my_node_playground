...
It provides an example of streaming data from a single streaming connection, Twitter, and sharing it between multiple clients that can connect to node concurrently.  It does this purely by emitting an event each time a tweet is received, and having each request respond to the event.
